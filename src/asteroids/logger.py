"""Simple JSONL logging helpers for game state and events."""
...
def log_state() -> None:
    """Write a lightweight game-state snapshot roughly once per second."""
    ...
    with open("game_state.jsonl", mode, encoding="utf-8") as file_handle:
...
def log_event(event_type: str, **details: Any) -> None:
    """Append a structured event entry to the event log."""
    ...
    with open("game_events.jsonl", mode, encoding="utf-8") as file_handle:
